...
import json
import datetime
import hashlib  # ✅ Add hashing
from config import Config  # ✅ Use your Flask secret
...
class Token:
    ...
    def fruuty_token(self,
        ...
        # ✅ Add signature only for money tokens
        if self.token_type == 'money':
            signature_string = f"{self.user_id}|{name}|{self.token_id}|{self.token_type}|fruutty|{Config.FLASK_SECRET_KEY}"
            signature_hash = hashlib.sha3_512(signature_string.encode('utf-8')).hexdigest()
            self.data['signature'] = signature_hash
...
# Test block
token = Token()
amount = token.fruuty_token(
    8000,
    name='TJFiles',
    token_id='T1234',
    user_id='U5678',
    token_type='money',  # ✅ Try product here to see difference
)
